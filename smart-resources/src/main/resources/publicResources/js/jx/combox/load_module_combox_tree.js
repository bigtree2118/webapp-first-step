/**
 * 定义区域下拉选择框树(ModuleComboxTree)
 * v1.0.0
 * author: chenbing
 */
function ModuleComboxTree(c){
	...
	this.onNodeClick = function (e, treeId, treeNode){
		//回填
		$("#" + this.nameFiledId).val(treeNode.module_name);
		$("#" + this.idFieldId).val(treeNode.module_id);
		this.hideTreeBox();
	};
	...
	this.load = function(){
		...
		$.ajaxSetup({async: false});
	 	$.getJSON(JXCore.basePath + 'basic/sys/sysModule_loadDictItemAll.action', params, function(json){
