/**
 * 定义流程下拉选择框树(PROINFOComboxTree)
 * v1.0.0
 * author: chenbing
 */
function BpmComboxTree(c){
	this.idFieldId = c.idFieldId;
	this.nameFiledId = c.nameFieldId;
	this.codeFildId = c.codeFildId;
	...
	this.onNodeClick = function (e, treeId, treeNode){
		...
		$("#" + this.codeFildId).val(node_id);
		$("#" + this.idFieldId).val(treeNode.ext_field1);
	...
	this.load = function(){
		...
		var temp_cache = null;
	 	$.getJSON(JXCore.basePath + 'workflow/base/process_loadTypeprodefTreeNode.action', params, function(json){
