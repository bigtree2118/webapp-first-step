...
$.extend($.fn.dialog.defaults, EasyuiExt.onMove);
$.extend($.fn.window.defaults, EasyuiExt.onMove);
$.extend($.fn.panel.defaults, EasyuiExt.onMove);
...
$.extend($.fn.datagrid.defaults, EasyuiExt.onLoadError);
$.extend($.fn.treegrid.defaults, EasyuiExt.onLoadError);
$.extend($.fn.tree.defaults, EasyuiExt.onLoadError);
$.extend($.fn.combogrid.defaults, EasyuiExt.onLoadError);
$.extend($.fn.combobox.defaults, EasyuiExt.onLoadError);
$.extend($.fn.form.defaults, EasyuiExt.onLoadError);
...
$.extend($.fn.combotree.defaults, EasyuiExt.loadFilter);
$.extend($.fn.tree.defaults, EasyuiExt.loadFilter);
 ...
 * 
 */
EasyuiExt.modalDialog = function(options) {
	var opts = $.extend({
		title : '&nbsp;',
		width : 700,
		height : 480,
 ...
 * @param themeName
 */
EasyuiExt.changeTheme = function(themeName) {
	...
	EasyuiExt.cookie('easyuiTheme', themeName, {
		expires : 7
	});
};
 ...
 * @requires jQuery,EasyUI
 */
EasyuiExt.progressBar = function(options) {
	if (typeof options == 'string') {
		if (options == 'close') {
			$('#easyuiProgressBarDiv').dialog('destroy');
		}
	} else {
		if ($('#easyuiProgressBarDiv').length < 1) {
			var opts = $.extend({
				title : '&nbsp;',
				closable : false,
				width : 300,
				height : 60,
				modal : true,
				content : '<div id="easyuiProgressBar" class="easyui-progressbar" data-options="value:0"></div>'
			}, options);
			$('<div id="easyuiProgressBarDiv"/>').dialog(opts);
			$.parser.parse('#easyuiProgressBarDiv');
		} else {
			$('#easyuiProgressBarDiv').dialog('open');
		}
		if (options.value) {
			$('#easyuiProgressBar').progressbar('setValue', options.value);
		}
	}
};

/**
 * 扩展easyui.datagrid组件增加编辑类型combogrid
 */
