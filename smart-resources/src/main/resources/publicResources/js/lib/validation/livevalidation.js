...
var Validate = {
    ...
    Presence: function(value, paramsObj){
    	...
    	if((typeof value) === 'string'){
    		if(value.replace( trimLeft, '' ).replace( trimRight, '' ).length == 0){
    			Validate.fail(message);
    		}
    	}
    	return true;
   ...
   *                           (DEFAULT: false)
    ...
    Telephone: function(value, paramsObj){
    	...
    	Validate.Format(value, { failureMessage: message, pattern: /(^[0-9]{3,4}\-[0-9]{3,8}$)|(^[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}1[3|5|8][0-9]{9}$)/ } );
    	return true;
    },
    /**
     *	简单电话号码（不包括：座机电话）验证
    ...
    Phone: function(value, paramsObj){
    	...
    	var paramsObj = paramsObj || {};
    	var message = paramsObj.failureMessage || "无效号码!";
    	Validate.Format(value, { failureMessage: message, pattern: /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/ } );
    	return true;
    },
    /**
     *	身份证号码验证
     ...
     * 16(19)位银行卡号校验
     */
    BankNumber: function(value, paramsObj){
    	var paramsObj = paramsObj || {};
    	var message = paramsObj.failureMessage || "无效卡号!";
    	Validate.Numericality(value,{onlyInteger:true});
		Validate.Length(value,{minimum:16,maximum:19});
		var lastNum = value.substr(value.length - 1, 1);
		var firstNum = value.substr(0, value.length - 1);
		var numSum = 0; var array = firstNum.split("");
		for ( var i = array.length-1, j = 0 ,k = 0; i >= 0; i--, j++) {
			k = parseInt(array[i]);
			if (j % 2 == 0) {
				k = k * 2; 
				if (k > 10)k = 1 + k % 10; 
		...
		var luhmNum = (parseInt(numSum) % 10 == 0) ? 0: parseInt(10 - parseInt(numSum) % 10);
     ...
     */
    CodeNo: function(value, paramsObj){
    	if(!value || value.length == 0){
    		return true;
    	}
    	var paramsObj = paramsObj || {};
    ...
    Acceptance: function(value, paramsObj){
      	var paramsObj = paramsObj || {};
    	var message = paramsObj.failureMessage || "必须选择一项!";
    	if(!value){ 
    	  	Validate.fail(message);
    	}
    	return true;
