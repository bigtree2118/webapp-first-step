...
public class SsoRealm extends AuthorizingRealm {
	 ...
	 * @param rpcUser
	 */
	private void recordLoginEvent(RpcUser rpcUser){
		try {
			UserLocalInfo userLocalInfo = null;
			if(rpcUser.getProfile() instanceof UserLocalInfo){
				userLocalInfo = (UserLocalInfo)rpcUser.getProfile();
				
				userLocalInfoService.saveOrUpdate(userLocalInfo);
			}
		} catch (Exception e) {
			//LOGGER.error("记录系统用户登录事件时，出现异常：" + e.getMessage());
		}
	}
	...
	public Set<String> invokePermissionInSession(String token) {
		...
		for (Menu menu : dbList) {
			...
			//将目录菜单或按钮的相关url添加到用户权限集合中
			if (StringUtils.isNotBlank(menu.getRela_url())){
